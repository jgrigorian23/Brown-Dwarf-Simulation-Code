...
Inputs:
(float) x - random value between 0 and 1, including 0, generated by random.random()
(float) m - slope of approximating line for inside out function (set in inside_out_time_inverse() as 0.03)
(float) b - y-intercepg of approximating line for inside out function (set in inside_out_time_inverse() as 0.36) 
Outputs:
(float) - Constant Inverse Age CDF Value [GYR]
"""
...
"""
Quick Look: Returns an list Inside Out Inverse Age CDF Values for a given integer.
    Called exclusively by main()
Inputs:
(int) n - number of samples in each list
Outputs:
...
"""
Quick Look: Returns an list Late Burst Inverse Age CDF Values for a given integer. Calls ins_f() and constf() since the late burst is a mixture of both.
    Called exclusively by main()
Inputs:
(int) n - number of samples in each list
Outputs:
...
Output:
(List) - 3 Lists of age, mass, and temperature respectively of the reference values from the source file, "sm_hybrid_age (1).txt".
"""
...
"""
Star Class:
Holds age, mass, and temperature of each star, with getter and setter methods for each.
"""
